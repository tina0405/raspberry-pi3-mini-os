...
#include <stdint.h>								// Needed for uint8_t, uint32_t, uint64_t etc								// Needed for string copy
#include "SDCard.h"	
#include "gpio.h"
#include "delays.h"
...
int wctob(uint16_t word){
     if (word >= L'\0' && word <= L'\x7f')
	return (int) word;
     return '\0';
...
****************************************************************************/
#define EMMC_ARG2			((volatile __attribute__((aligned(4))) uint32_t*)(uintptr_t)(MMIO_BASE + 0x300000))
#define EMMC_BLKSIZECNT		((volatile struct __attribute__((aligned(4))) regBLKSIZECNT*)(uintptr_t)(MMIO_BASE + 0x300004))
#define EMMC_ARG1			((volatile __attribute__((aligned(4))) uint32_t*)(uintptr_t)(MMIO_BASE + 0x300008))
#define EMMC_CMDTM			((volatile struct __attribute__((aligned(4))) regCMDTM*)(uintptr_t)(MMIO_BASE + 0x30000c))
#define EMMC_RESP0			((volatile __attribute__((aligned(4))) uint32_t*)(uintptr_t)(MMIO_BASE + 0x300010))
#define EMMC_RESP1			((volatile __attribute__((aligned(4))) uint32_t*)(uintptr_t)(MMIO_BASE + 0x300014))
#define EMMC_RESP2			((volatile __attribute__((aligned(4))) uint32_t*)(uintptr_t)(MMIO_BASE + 0x300018))
#define EMMC_RESP3			((volatile __attribute__((aligned(4))) uint32_t*)(uintptr_t)(MMIO_BASE + 0x30001C))
#define EMMC_DATA			((volatile __attribute__((aligned(4))) uint32_t*)(uintptr_t)(MMIO_BASE + 0x300020))
#define EMMC_STATUS			((volatile struct __attribute__((aligned(4))) regSTATUS*)(uintptr_t)(MMIO_BASE + 0x300024))
#define EMMC_CONTROL0		((volatile struct __attribute__((aligned(4))) regCONTROL0*)(uintptr_t)(MMIO_BASE + 0x300028))
#define EMMC_CONTROL1		((volatile struct __attribute__((aligned(4))) regCONTROL1*)(uintptr_t)(MMIO_BASE + 0x30002C))
#define EMMC_INTERRUPT		((volatile struct __attribute__((aligned(4))) regINTERRUPT*)(uintptr_t)(MMIO_BASE + 0x300030))
#define EMMC_IRPT_MASK		((volatile struct __attribute__((aligned(4))) regIRPT_MASK*)(uintptr_t)(MMIO_BASE + 0x300034))
#define EMMC_IRPT_EN		((volatile struct __attribute__((aligned(4))) regIRPT_EN*)(uintptr_t)(MMIO_BASE + 0x300038))
#define EMMC_CONTROL2		((volatile struct __attribute__((aligned(4))) regCONTROL2*)(uintptr_t)(MMIO_BASE + 0x30003C))
#define EMMC_TUNE_STEP 		((volatile struct __attribute__((aligned(4))) regTUNE_STEP*)(uintptr_t)(MMIO_BASE + 0x300088))
#define EMMC_SLOTISR_VER	((volatile struct __attribute__((aligned(4))) regSLOTISR_VER*)(uintptr_t)(MMIO_BASE + 0x3000fC))
...
static SDRESULT sdWaitForInterrupt (uint32_t mask ) 
{
	...
	while (!(EMMC_INTERRUPT->Raw32 & tMask) && (td < 1000000)) {
		if (!start_time) start_time = get_system_timer();					// If start time not set the set start time
			else td = get_system_timer() - start_time;			// Time difference between start time and now
	}
...
static SDRESULT sdWaitForCommand (void) 
{
	...
	{
		if (!start_time) start_time = get_system_timer();					// If start time not set the set start time
			else td = get_system_timer() - start_time;			// Time difference between start and now
	}
...
static int sdSendCommandP( EMMCCommand* cmd, uint32_t arg )
{
	...
	LOG_DEBUG("EMMC: Sending command %s code %08x arg %x\n\r",
	...
	*EMMC_CMDTM = cmd->code;										// Send command to SD card								
	if ( cmd->delay ) wait_msec(cmd->delay);						// Wait for required delay
...
static SDRESULT sdReadSCR (void)
{
	...
	while( numRead < 2 )  {
		if (EMMC_STATUS->READ_TRANSFER) {
			...
			if (numRead == 0) {sdCard.scr.Raw32_Lo = *EMMC_DATA; sd_scr[numRead] = sdCard.scr.Raw32_Lo;}
				else{ sdCard.scr.Raw32_Hi = *EMMC_DATA;  sd_scr[numRead] =sdCard.scr.Raw32_Hi;}
			numRead++;
		} else {
			wait_msec(1);
			if( --count == 0 ) break;
	...
	{
		if (LOG_ERROR) {
			LOG_ERROR("EMMC: SEND_SCR ERR: %x %x %x\n\r", 
...
static SDRESULT sdSetClock (uint32_t freq)
{
	...
	if (td >= 100000) {												// Timeout waiting for inghibit flags
		if (LOG_ERROR) LOG_ERROR("EMMC: Set clock: timeout waiting for inhibit flags. Status %0x.\n\r",
			(unsigned int)EMMC_STATUS->Raw32);
	...
	wait_msec(10);													// We must now wait 10 microseconds
...
static SDRESULT sdResetCard (void)
{
	...
	wait_msec(10);													// Wait 10 microseconds
...
SDRESULT sdClearBlocks(uint32_t startBlock , uint32_t numBlocks)
{
	...
	if ( --count == 0 )
		{
		if (LOG_ERROR) LOG_ERROR("EMMC: Timeout waiting for erase: %x %x\n\r", 
			(unsigned int)EMMC_STATUS->Raw32, (unsigned int)EMMC_INTERRUPT->Raw32);
		return SD_TIMEOUT;
		}
...
static bool LoadDrivePartition (printhandler prn_basic) {
	...
	//	if (prn_basic) prn_basic("FAT32 Volume Label: %s, ID: %x\n\r", 
		...
		if (bpb->FSTypeData.fat1612.BS_BootSig == 0x29) {
			if (prn_basic) prn_basic("FAT12/16 Volume Label: %s, Volume ID %x\n\r", 
...
#define EMMC_SLOTISR_VER_1    ((volatile unsigned int*)(MMIO_BASE+0x003000FC))
SDRESULT sdInitCard (printhandler prn_basic, printhandler prn_error, bool mount)
{
	...
	LOG_ERROR = prn_error;						// Hold the provided print error handler
	 sd_hv = (*EMMC_SLOTISR_VER_1 & HOST_SPEC_NUM) >> HOST_SPEC_NUM_SHIFT;
	// Reset the card.
	...
	if( (resp = sdAppSendOpCond(ACMD41_ARG_HC)) ) return sdDebugResponse(resp);
        if(sdCard.rca&ACMD41_CMD_CCS) ccs=SCR_SUPP_CCS;
